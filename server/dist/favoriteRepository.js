...
async function invalidateUserFavoritesCache(userId) {
...
export async function addFavorite(pool, userId, venueId) {
    // Check if the favorite already exists
    const existing = await pool.query('SELECT * FROM favorite_venues WHERE user_id = $1 AND venue_id = $2', [userId, venueId]);
    if (existing.rows.length > 0) {
        return null; // Indicates a conflict
    }
    const res = await pool.query('INSERT INTO favorite_venues (user_id, venue_id) VALUES ($1, $2) RETURNING *', [userId, venueId]);
    if (res.rows[0]) {
        await invalidateUserFavoritesCache(userId); // Invalidate cache on add
    }
    return res.rows[0];
}
export async function removeFavorite(pool, userId, venueId) {
    const res = await pool.query('DELETE FROM favorite_venues WHERE user_id = $1 AND venue_id = $2 RETURNING *', [userId, venueId]);
    if (res.rows[0]) {
        await invalidateUserFavoritesCache(userId); // Invalidate cache on remove
    }
    return res.rows[0];
}
export async function getFavorites(pool, userId, limit, offset) {
    ...
    const venuesRes = await pool.query(`SELECT v.* FROM venues v JOIN favorite_venues f ON v.id = f.venue_id WHERE f.user_id = $1 ORDER BY v.name ASC LIMIT $2 OFFSET $3`, [userId, limit, offset]);
    // Query to get total count of favorite venues for the user
    const countRes = await pool.query(`SELECT COUNT(*) FROM favorite_venues WHERE user_id = $1`, [userId]);
